...
public class CatalogAnnotationProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations,
        ...
        if (foundMethod) { // only write file if we actually found something
            String outputFile = System.getProperty("catalog.output.file", "cucumber-catalog.xml");
    ...
    public void addStep(Element elem) {
        ...
        processingEnv.getMessager().printMessage(Diagnostic.Kind.NOTE,
                String.format("Found step method: %s.%s.%s", packageName, className, step.method));
    }
...
//    }
    ...
    private String className(Element elem) {
        ...
        if (parent instanceof TypeElement) {
            ...
            throw new RuntimeException("Expected enclosing element to be TypeElement");
        }
    }
    ...
    private static final Pattern SOURCE_FORMAT = Pattern.compile("^\\w+\\[(.+)\\]:(\\d+)$");

    private String sourceLoc(Element elem) {
        Elements utils = processingEnv.getElementUtils();
        if (utils instanceof JavacElements) {
            Object src = ((JavacElements) utils).getSourcePosition(elem);
            Matcher m = SOURCE_FORMAT.matcher(src.toString());
            if (m.matches()) {
                return m.group(1) + ":" + m.group(2);
            } else {
                throw new RuntimeException("Source line did not match expected format");
            }
        } else {
